...
rule all:
    input: expand("output/{name}/{logic}/results.json", logic=logic, name=name)
...
rule cello:
    ...
    output: "output/{name}/{logic}/results.json"
    benchmark: "{name}/{logic}/benchmark.txt"
    params: app_path=config['app_path'],
            out_dir=lambda wildcards: f'output/{wildcards.name}/{wildcards.logic}'
    shell:
         """
